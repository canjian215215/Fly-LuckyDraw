...
const DragHandle = SortableHandle(() => <FontAwesomeIcon icon={faBars} size={'1x'} className={'drag-handle'}/>);
  ...
  const SortableItem = SortableElement(({ value, sortIndex }) =>
    <div key={sortIndex} className="lottery-item">
      ...
      <div className="lottery-item-count">
        <input
          type={'number'}
          defaultValue={value.totalNum}
          onBlur={e => {
            items.find(lottery => lottery.id === value.id).totalNum = parseInt(e.target.value)
            onItemChange(items)
          }}
        />
      </div>
